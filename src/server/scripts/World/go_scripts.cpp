        ...
        struct go_dark_iron_pillowAI : public GameObjectAI
        {
            ...
            bool GossipHello(Player* player) override
            {
                ...
                if (!player->FindNearestGameObject(GAMEOBJECT_DARK_IRON_PILLOW_EVENT_GENERATOR, 200.0f))
                {
                    ...
                    player->SummonGameObject(GAMEOBJECT_DARK_IRON_PILLOW_EVENT_GENERATOR, pos, QuaternionData(), 0, GO_SUMMON_TIMED_DESPAWN);
                }
...
class go_dark_iron_pillow_event_generator : public GameObjectScript
{
    public:
        ...
        struct go_dark_iron_pillow_event_generatorAI : public GameObjectAI
        {
            ...
            void InitializeAI() override
            {
                events.ScheduleEvent(EVENT_WAVE_1,       60 * IN_MILLISECONDS, 0, 0); 
                events.ScheduleEvent(EVENT_WAVE_2,      180 * IN_MILLISECONDS, 0, 0); 
                events.ScheduleEvent(EVENT_CHAMBERMAID, 300 * IN_MILLISECONDS, 0, 0); 
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_WAVE_1:
                        ...
                        {
                            if (TempSummon* summon = owner->SummonCreature(NPC_SLEEPY_DARK_IRON_WORKER, -6568.03f, -1306.73f, 208.70f, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 120 * IN_MILLISECONDS))
                            {
                                summon->ToCreature()->GetMotionMaster()->MovePath(NPC_SLEEPY_DARK_IRON_WORKER * 100 + i, true);
                                if (i == 0)
                                    summon->ToCreature()->AI()->Talk(SAY_IRON_WORKER_0);
                            }
                        }
                        break;
                    case EVENT_WAVE_2:
                        for (size_t i = 0; i < 10; i++)
                        {
                            if (TempSummon* summon = owner->SummonCreature(NPC_SLEEPY_DARK_IRON_WORKER, -6481.22f, -1349.06f, 212.10f, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 120 * IN_MILLISECONDS))
                            {
                                summon->ToCreature()->GetMotionMaster()->MovePath(NPC_SLEEPY_DARK_IRON_WORKER * 100 + 10 + i, true);
    
                                if (i == 0)
                                    summon->ToCreature()->AI()->Talk(SAY_IRON_WORKER_1);
                            }
                        }
                        break;
                    case EVENT_CHAMBERMAID:
                        owner->SummonCreature(NPC_CHAMBERMAID_PILLACLENCHER, -6545.88f, -1345.29f, 208.89f, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 300 * IN_MILLISECONDS);
                        owner->Delete();
                        break;
                    }
                }
